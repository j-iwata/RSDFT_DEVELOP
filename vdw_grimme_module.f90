...
CONTAINS
  ...
  SUBROUTINE read_vdw_grimme( rank, unit )
    implicit none
...
999    continue
  ...
  SUBROUTINE init_vdw_grimme(xctype,aa_in,Natm,np,myrnk,ki_in,z_in)
    implicit none
    character(*),intent(IN) :: xctype
    real(8),intent(IN) :: aa_in(3,3)
    integer,intent(IN) :: Natm, np, myrnk
    ...
    call prep_atom_parallel( Natm, np, myrnk, MI_0, MI_1 )
...
!-----------------------------------------
  ...
  SUBROUTINE calc_E_vdw_grimme( MI, asi, Edisp_out )
    implicit none
    integer,intent(IN)  :: MI
    real(8),intent(IN)  :: asi(3,MI)
    real(8),optional,intent(OUT) :: Edisp_out
    integer :: i,j,m1,m2,m3,l,ierr
    real(8) :: Edisp
...
!        if ( abs(m1)<l .and. abs(m2)<l .and. abs(m3)<l ) cycle
  ...
  SUBROUTINE calc_F_vdw_grimme( MI, asi, Fdisp )
    implicit none
    integer,intent(IN)  :: MI
    real(8),intent(IN)  :: asi(3,MI)
    real(8),intent(INOUT) :: Fdisp(3,MI)
    integer :: i,j,m1,m2,m3,l
    real(8) :: Rix,Riy,Riz,Rjx,Rjy,Rjz,Rij
    real(8) :: C6i,C6j,C6ij,R0i,R0j,fdmp,Rr
...
!- solve eigenvalue problem by power method
  ...
  SUBROUTINE prep_atom_parallel(n,np,myrnk,n0,n1)
    implicit none
    integer,intent(IN)  :: n,np,myrnk
  ...
  END SUBROUTINE prep_atom_parallel
