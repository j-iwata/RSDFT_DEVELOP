!--------1---------2---------3---------4---------5---------6---------7--
      ...
      integer :: id1,id2,id3,iii1,iii2,iii3,ML0,irlma
      integer :: a1b,a2b,a3b,b1b,b2b,b3b,nreq
      integer :: mm1,mm2,mm3,mm4,m8,m9,m0,ibuf(2,3)
      integer :: MMJJ_0,MMJJ,lma0,nzlma_0,isym
      ...
      if (DISP_SWITCH) then
         write(*,'(a60," prep_ps_oh_sym")') repeat("-",60)
      end if
...
!-----------------------------------------------------------------
      ...
      call mpi_allreduce(rwork(:,:,:,1),rwork(:,:,:,2),m0,mpi_real8,mpi_sum,MPI_COMM_WORLD,ierr)
...
! ----------------------------------------------------------------------
      ...
      if ( Mlma<nprocs ) then
         ...
         nzlma_0 = min(Mlma/nprocs*27,Mlma)
      end if
      if ( DISP_SWITCH ) then
...
!- allocate ------------------------------------------------------------
         ...
         allocate( icheck_tmp1(0:nprocs-1)         ) ; icheck_tmp1=0
         allocate( rtmp3(MMJJ_0,nzlma_0) ) ; rtmp3=0.d0
         allocate( rrr(-mm1:mm1,-mm2:mm2,-mm3:mm3,2*L+1,2) ) ; rrr=0.d0
...
!-----------------------------------------------------------------------
      ...
      do irank=0,nprocs-1
         do i=idisp(irank)+1,idisp(irank)+ircnt(irank)
            do isym=1,nsym
      ...
      do a=1,MI
         ...
         do iorb=1,norb(ik)
            ...
            do i=1,M_grid_ion
               ...
               do ii1=ll_0,Nintp
                  ...
                  if ( irank<0 .or. nprocs<=irank ) cycle
               ...
               if ( n1<=LLL(id1,id2,id3) .and. LLL(id1,id2,id3)<=n2 ) then
                  ...
                  do j1=0,Ndense-1
                     ...
                     do m=1,2*L+1
                        ...
                        do ii1=ll_0,Nintp
                           iii1=i1+ii1
                           rrr(iii1,iii2,iii3,m,1)=rrr(iii1,iii2,iii3,m,1)+tmp2*Clag1(ii1,j1)
                           end do
                           end do
                           end do
                        end do
               ...
               if ( icheck_tmp1(myrank)>0 ) then

                  call mpi_allreduce_nlpp( rrr(:,:,:,1,1),rrr(:,:,:,1,2),m0,icheck_tmp1,ierr,"R" )
                  ...
                  do i1=-mm1,mm1
                     id1=i1+ic1
                     if ( PPP(id1,id2,id3)==myrank ) then
                        lma = lma0
                        do m=1,2*L+1
                           if ( abs(rrr(i1,i2,i3,m,2))<1.d-10 ) cycle
                           j1=j1+1
                           do mm=1,2*L+1
                              lma=lma+1
                              rtmp3(j1,lma)=rrr(i1,i2,i3,mm,2)
                              jtmp3(1,j1,lma)=id1
                              jtmp3(2,j1,lma)=id2
                              jtmp3(3,j1,lma)=id3
                              jtmp3(4,j1,lma)=i1
         ...
         ibuf(1,1)=MMJJ
         ibuf(2,1)=nzlma
         call mpi_allreduce(ibuf(1,1),ibuf(1,2),2,mpi_integer,mpi_min,mpi_comm_world,ierr)
         call mpi_allreduce(ibuf(1,1),ibuf(1,3),2,mpi_integer,mpi_max,mpi_comm_world,ierr)

         if (DISP_SWITCH) then
            write(*,*) "lma=",lma
            write(*,*) "myrank,nzlma,MMJJ",myrank,nzlma,MMJJ
...
!---------------------------------------------------------------------
         ...
         call mpi_allreduce(nzlma,nzlma_0,1,mpi_integer,mpi_max,mpi_comm_world,ierr)

         nzlma_0 = min(nzlma_0*2,Mlma)
         nrlma   = 0

!- allocate ----------------------------------------------------------
         allocate( lma_nsend_tmp(0:nprocs-1) )
         allocate( icheck_tmp1(0:nprocs-1)   )
         allocate( icheck_tmp2(0:nprocs-1)   )
         allocate( nl_rank_map_tmp(0:nprocs) )
         allocate( maps_tmp(nzlma_0,6) )
         allocate( sendmap0(nzlma_0,0:nprocs-1) )
         allocate( recvmap0(nzlma_0,0:nprocs-1) )
         n=max(np1,np2,np3)
         allocate( itmp(n,3) ) ; itmp=0
         mem=mem+bsintg*(nprocs*3+nprocs+1+nzlma_0*6)
         mem=mem+bsintg*(nzlma_0*nprocs*2+n*3)
         memax=max(mem,memax)
!---------------------------------------------------------------------
...
&                             ,icheck_tmp1,1,mpi_integer,mpi_comm_world,ierr)

            if ( icheck_tmp1(myrank)/=0 ) then
               icheck_tmp2(myrank)=icheck_tmp2(myrank)+1
               maps_tmp(icheck_tmp2(myrank),1)=icheck_tmp1(myrank)
               maps_tmp(icheck_tmp2(myrank),2)=inorm(iorb,ik)
               maps_tmp(icheck_tmp2(myrank),3)=a
               maps_tmp(icheck_tmp2(myrank),4)=L
               maps_tmp(icheck_tmp2(myrank),5)=m
               maps_tmp(icheck_tmp2(myrank),6)=iorb
               do n=0,nprocs-1
                  if ( n==myrank .or. icheck_tmp1(n)==0 ) cycle
                  ...
                  sendmap0(lma_nsend_tmp(n),n)=icheck_tmp2(myrank)
         ...
         nzlma = icheck_tmp2(myrank)

!- allocate ---------------------
         call gv_alloc("MJJ")
!--------------------------------

         allocate( itmp1(n1:n2) ) ; mem=mem+bsintg*(n2-n1+1) ; memax=max(mem,memax)
         ...
         do lma=1,nzlma
            ...
            do i=1,mtmp3(l)
               ...
               i3=jtmp3(3,i,l)
               j=LLL(i1,i2,i3)
               if ( n1<=j .and. j<=n2 ) itmp1(j)=itmp1(j)+1
            end do
            MJJ(lma)=count( itmp1/=0 )
         end do

         deallocate( itmp1 ) ; mem=mem-bsintg*(n2-n1+1)

         MAXMJJ = maxval( MJJ(1:nzlma) )

         ibuf(1,1)=MAXMJJ
         call mpi_allreduce(ibuf(1,1),ibuf(1,2),1,mpi_integer,mpi_min,mpi_comm_world,ierr)
         call mpi_allreduce(ibuf(1,1),ibuf(1,3),1,mpi_integer,mpi_max,mpi_comm_world,ierr)
         if (DISP_SWITCH) then
...
!--------------------------------------------
         ...
         allocate( itmp1(0:nprocs-1) ) ; itmp1=0
         itmp1(myrank)=nl_max_send
         call mpi_allgather(nl_max_send,1,mpi_integer,itmp1,1,mpi_integer,mpi_comm_world,ierr)
         if (DISP_SWITCH) then
...
!- allocate -------------------------------------------------
         allocate( ireq(2*nprocs) )
         mem=mem+bsintg*2*nprocs ; memax=max(mem,memax)
!------------------------------------------------------------

         nreq=0
         do n=0,nprocs-1
            if ( lma_nsend(n)<=0 .or. n==myrank ) cycle
            nreq=nreq+1
            call mpi_isend(recvmap0(1,n),lma_nsend(n),mpi_integer,n,1,mpi_comm_world,ireq(nreq),ierr)
            nreq=nreq+1
            call mpi_irecv(recvmap(1,n) ,lma_nsend(n),mpi_integer,n,1,mpi_comm_world,ireq(nreq),ierr)
         end do
         call mpi_waitall(nreq,ireq,istatus,ierr)

!- deallocate -----------------------------------------------------
         deallocate( ireq ) ; mem=mem-bsintg*2*nprocs
!------------------------------------------------------------------

!- deallocate -----------------------------------------------------
         deallocate( recvmap0,sendmap0,lma_nsend_tmp )
         mem=mem-bsintg*(nzlma_0*nprocs*2 + nprocs)
!------------------------------------------------------------------
...
!--------------------------------
      ...
      if (DISP_SWITCH) then
         write(*,*) "TIME(PREP_PS_OH_SYM)=",ctime1-ctime0,etime1-etime0
         write(*,*) "MEM(MB)=",mem,memax*B2MB
      end if

      return

 900  call stop_program1("prep_ps_oh_sym",1)

      END SUBROUTINE prep_ps_sym
