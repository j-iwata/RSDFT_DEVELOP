MODULE ps_nloc_gth_module
  ...
  PRIVATE
  PUBLIC :: prep_ps_nloc_gth, init_force_ps_nloc_gth, init_ps_nloc_gth
...
CONTAINS
  ...
  SUBROUTINE prep_ps_nloc_gth(natm,n_0,L_0,MMJJ_0,M_grid_ion,map_grid_ion &
    ...
    do a=1,Natom
       ...
       do iorb=1,norb(ik)
          ...
          const1 = sqrt(hnl(n,L,ik)) &
                  *sqrt(2.d0)/(Rps0(iorb,ik)**(L+2*n-0.5d0)*sqrt(gamma))
...
!          end if
  ...
  END SUBROUTINE prep_ps_nloc_gth


  SUBROUTINE init_ps_nloc_gth(disp_switch)
    implicit none
  ...
  END SUBROUTINE init_ps_nloc_gth


  SUBROUTINE init_force_ps_nloc_gth &
...
!$OMP            ,yy1,yy2,yy3,tmp0,tmp1,lma,j,L1,L1z,cnst1,cnst2,cnst3,e,rp )
    do lma=1,nzlma
       ...
       cnst1=cnst*sqrt(hnl(n,L,ik))*sqrt(2.d0) &
            /(Rps0(iorb,ik)**(L+2*n-0.5d0)*sqrt(gamma))
...
!$OMP end do
  ...
  END SUBROUTINE init_force_ps_nloc_gth


END MODULE ps_nloc_gth_module
