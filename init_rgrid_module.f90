MODULE init_rgrid_module
  ...
  PRIVATE
  PUBLIC :: init_rgrid, parallel_init_rgrid

CONTAINS


  SUBROUTINE init_rgrid( SYStype, Md, unit )
    implicit none
    integer,intent(IN) :: SYStype, Md, unit
    ...
    case(1) ! ----- MOL Mol mol
       ...
       call Read_RgridMol(myrank,unit) ! set new parameters from external file
    ...
    if ( disp_switch_parallel ) then
       write(*,'(a60," init_rgrid")') repeat("-",60)
  ...
  SUBROUTINE parallel_init_rgrid(SYStype)
    implicit none
    ...
    case(0) ! ----- SOL sol
       ...
       call ParallelInit_RgridSol( node_partition, np_grid, pinfo_grid )

    case(1) ! ----- MOL mol

       call ParallelInit_RgridMol( node_partition, np_grid, pinfo_grid, myrank==0 )
    ...
    case(2)
       ...
       call ParallelInit_RgridSol( node_partition, np_grid, pinfo_grid )
       call ParallelInit_RgridESM( np_grid, pinfo_grid, Nshift )
  ...
  END SUBROUTINE parallel_init_rgrid


END MODULE init_rgrid_module
