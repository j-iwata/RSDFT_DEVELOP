...
contains
  ...
  subroutine atomopt_rf2( SYStype_in, fmax_tol, ncycle, okstep_in, max_linmin_in, max_alpha_in, NiterSCF_in )
    ...
    real(8) :: dxdf,dxHdx,c1,c2,c3,c,dmax,dtmp
    ...
    real(8),allocatable :: f(:),f0(:)
    real(8),allocatable :: dx(:),dg(:),Hdx(:),df(:)
    ...
    real(8),allocatable :: d_ref(:), d_diis(:)
    ...
    real(8) :: phi,phi1,phi2,phi3
    character(40) :: msg
    real(8) :: factor, tau, fmax_min, etot_min
    integer :: icount, istep_golden_search
    integer :: myrank, idiis
    include 'mpif.h'
    integer,allocatable :: ipiv(:)
    real(8) :: tol_lapack, cos_theta, cos_theta_threshold(2:10)

    call MPI_Comm_rank( MPI_COMM_WORLD, myrank, ierr )

    call write_border( 0, "atomopt_rf2(start)" )
...
! ---
    ...
    cos_theta_threshold = (/ 0.97d0, 0.84d0, 0.71d0, 0.67d0, 0.62d0 &
                            ,0.56d0, 0.49d0, 0.41d0, 1.0d-3 /)
...
! ---
      ...
      if ( loop == 1 ) then
        ...
        dxdf = sum( dx*df )
        if ( disp_sw ) write(*,*) "dxdf=",dxdf,sum(dx**2),sum(df**2)

! ---

        !if ( dxdg <= 0.0d0 .or. flag_forced_sd ) then
        if ( flag_forced_sd ) then
          ...
          do i=1,n
            Hessian(i,j) = Hessian(i,j) &
                 + phi*( -(df(i)+Hdx(i))*(df(j)+Hdx(j))*c3 ) &
                 + (1.0d0-phi)*( -df(i)*df(j)*c1 - Hdx(i)*Hdx(j)*c2 )
          end do
          end do
...
!            allocate( work(LWORK) ); work=0.0d0
!          end if
!          call dsyevx('V','I','U',n+1,Htmp,n+1,vl,vu,il,iu,tol_lapack,m,w,z,n+1 &
...
!          d(:) = Htmp(1:n,1)/Htmp(n+1,1)
          ...
          do i = 1, n
            Hessian(i,i) = Hessian(i,i) - w(1) + 1.d-8
          end do

! ---
!          Htmp(1:n,1:n) = Hessian(:,:)
!          call dsyev('V','U',n,Htmp,n+1,w,work,size(work),ierr)
!          do i=1,n
...
!          Hessian=matmul( Htmp,Vtmp )
! ---

          Vtmp = Hessian
          d = f
          call DGESV( n, 1, Vtmp, n, ipiv, d, n, ierr )

!          d_ref = d

        end if !flag_forced_sd

      end if ! ( loop == 1, loop > 1 )
...
! ---
      ...
      do linmin = linmin_start, max_linmin+1 !---------- Line Minimization
        ...
        if ( myrank == 0 ) call write_linmin( linmin )
...
999 call check_disp_switch( disp_sw, 1 )
    call write_border( 0, "atomopt_rf2(end)" )
  ...
  contains
    ...
    subroutine write_linmin(linmin)
      implicit none
      integer,intent(in) :: linmin
      integer,parameter :: u=298
      open(u,file='linmin_rf2.dat',form='unformatted')
      write(u) linmin
      write(u) info_linmin
      write(u) xyzf_linmin
      ...
      write(u) flag_sd
      close(u)
    end subroutine write_linmin

    subroutine read_linmin( linmin_start )
      implicit none
      integer,optional,intent(inout) :: linmin_start
      integer,parameter :: u=298
      integer :: linmin
      logical :: flag
      inquire( FILE='linmin_rf2.dat', EXIST=flag )
      if ( .not.flag ) return
      ...
      read(u) flag_sd
      close(u)
    end subroutine read_linmin

  end subroutine atomopt_rf2
  ...
  subroutine write_trajectory( xyz, force, msg )
    ...
    if ( myrank == 0 ) then
      !
      open(u,file='trajectory_rf2.xyz',position='append')
      call write_xyz_atom( u, xyz, "xyz_id_etot :"//msg )
      close(u)
      !
      open(u,file='trajectory_rf2.force',position='append')
