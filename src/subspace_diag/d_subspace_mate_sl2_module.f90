  ...
  subroutine d_subspace_mate_sl2( k, s, u )
    ...
    do ns = nb_0, nb_1, nblk
      ...
      do ms = ne+1, nb, nblk
        ...
        mm = me - ms + 1
        allocate( uhu(ms:me,ns:ne) ); uhu=z0
        call DGEMM('T','N',mm,nn,mg,zdV,u(1,ms),mg,hu,mg,z0,uhu,mm)
        echk2(ms:me,ns:ne) = uhu
        deallocate( uhu )
      end do !ms
    ...
    do ns = n1, n2, nblk
      ...
      do ms = m1, m2, nblk
        ...
        if ( ms == ns ) then
          ...
          if ( mm <= nblk_min ) then
            do n = ns, ne
              call DGEMV('T',mg,ne-n+1,zdV,u(1,n),mg,hu(1,n-n1+1),1,z0,uhu(n,n),1)
            end do
          else
          ...
          call DGEMM('T','N',mm,nn,mg,zdV,u(1,ms),mg,hu(1,jb1),mg,z0,uhu(ms,ns),ld)
