module force_ps_nloc2_hp_module

  use array_bound_module, only: MB_0,MB_1,MBZ_0,MBZ_1,MSP_0,MSP_1,ML_0
  use ps_nloc2_variables, only: nzlma, MMJJ, mmap, lmap, amap, &
       iorbmap, JJ_MAP, MJJ_MAP, JJP, MJJ, d_uVk, nrlma_xyz, iuV, lma_nsend, &
       num_2_rank, recvmap, sendmap, d_sbufnl3, d_rbufnl3, Mlma, TYPE_MAIN, zero, &
...
contains
  ...
  subroutine calc_force_ps_nloc2_hp( force2 )
...
#endif
    real(8),allocatable :: wd0(:,:,:,:)
    complex(8),allocatable :: wz0(:,:,:,:)
...
!$omp end master
    ...
    do s=MSP_0,MSP_1
    do k=MBZ_0,MBZ_1
!!$OMP do schedule(dynamic) private( c,i,d1,d2,d3,kr,ztmp,i1,i2,i3,n,j,lma )
!$OMP do private( c,i,d1,d2,d3,kr,ztmp,i1,i2,i3,n,j,lma )
    do n=MB_0,MB_1
...
!        else ! --- MJJ(lma) /= MJJ_MAP(lma) ---
        ...
        do j=1,MJJ(lma)
          i=JJP(j,lma)
          wtmp5(0,lma,n,k,s)=wtmp5(0,lma,n,k,s) &
               +d_uVk(j,lma,k)*conjg(unk(i,n,k,s))
        end do
        wtmp5(0,lma,n,k,s)=iuV(lma)*c*wtmp5(0,lma,n,k,s)
        do j=1,MJJ_MAP(lma)
          i1=JJ_MAP(1,j,lma)
...
#ifdef _DRSDFT_
        call d_ps_nloc2_comm( wtmp5(:,:,ib1:ib2,k,s) )
#endif
      else
#ifndef _DRSDFT_
        call z_ps_nloc2_comm( wtmp5(:,:,ib1:ib2,k,s) )
#endif
      end if
...
!                         ,sbufnl,rbufnl,nzlma,ib1,ib2,wtmp5(0,1,ib1,k,s))
      ...
      !         d_sbufnl3(i1,m,i)=vtmp2(i3,sendmap(i2,irank),ib)
      !       end do
      !       end do
      !       end do
      !       nreq=nreq+1
      ...
      !       !     ,TYPE_MAIN,irank,1,comm_grid,ireq(nreq),ierr)
      !       call MPI_Isend(d_sbufnl3(1,m,i),4*lma_nsend(irank)*nnn &
      !                ,TYPE_MAIN,irank,1,comm_grid,ireq(nreq),ierr)
      !     end if
      !     if( jrank >= 0 )then
      !       nreq=nreq+1
      ...
      !       !     ,TYPE_MAIN,jrank,1,comm_grid,ireq(nreq),ierr)
      !       call MPI_Irecv(d_rbufnl3(1,m,i),4*lma_nsend(jrank)*nnn &
      !            ,TYPE_MAIN,jrank,1,comm_grid,ireq(nreq),ierr)
      !     end if
      ...
      !         wtmp5(i3,recvmap(i2,jrank),ib,k,s) &
      !              =wtmp5(i3,recvmap(i2,jrank),ib,k,s)+d_rbufnl3(i1,m,i)
      !       end do
      !       end do
      !       end do
      !     end if
...
!$OMP single
    call MPI_Allreduce(work2,force2,3*Natom,MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ierr)
    deallocate( work2 )
    deallocate( a_rank )
    deallocate( vtmp2 )
    deallocate( wtmp5 )
...
end module force_ps_nloc2_hp_module
