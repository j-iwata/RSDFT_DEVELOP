module ps_local_ffte_module

  use ggrid_module, only: construct_Ggrid,destruct_Ggrid,LLG,MGL,allgatherv_Ggrid
  use ffte_sub_module, only: zwork1_ffte,zwork2_ffte,npuy,npuz,comm_ffty,comm_fftz
  use watch_module

  implicit none

  private
  public :: init_ps_local_ffte
...
contains
  ...
  subroutine init_ps_local_ffte( Ngrid, Igrid )
    implicit none
    ...
    call write_border( 0, ' init_ps_local_ffte(start)' )
    ...
    call write_border( 0, ' init_ps_local_ffte(end)' )

  end subroutine init_ps_local_ffte

  subroutine construct_ps_local_ffte( vqlg, SGK, Vion )
    implicit none
    ...
    call write_border( 0, " construct_ps_local_ffte(start)" )
    if ( .not.flag_init_done ) call stop_program('Call "init_ps_local_ffte" first.')
...
!      write(*,*) "time(construct_ps_loc_ffte_1)",ctt(1)-ctt(0),ett(1)-ett(0)
!      write(*,*) "time(construct_ps_loc_ffte_2)",ctt(2)-ctt(1),ett(2)-ett(1)
!      write(*,*) "time(construct_ps_loc_ffte_3)",ctt(3)-ctt(2),ett(3)-ett(2)
!    end if

    call write_border( 0, " construct_ps_local_ffte(end)" )

  end subroutine construct_ps_local_ffte

end module ps_local_ffte_module
