module construct_vion_vh_floc_module
  ...
  use parallel_module, only: RSDFT_MPI_COMPLEX16, comm_grid, myrank &
                            ,construct_id_ir_parallel
  ...
  public :: construct_vion_vh_floc_2
...
! ---
    ...
    do a=MI_0,MI_1
       ...
       zsum2=z0
       zsum3=z0
!$OMP parallel do reduction(+:zsum1,zsum2,zsum3) private( i1,i2,i3,Gx,Gy,Gz,Gr,j,ztmpv,ztmpf )
       do i=1,size(MGL)
          i1=LLG(1,i)
          i2=LLG(2,i)
          i3=LLG(3,i)
...
!$OMP end parallel do
    ...
    !call watchb( ttmp, ttt(:,14), barrier='on' )

    !if ( myrank == 0 ) then
    !   do i=1,size(ttt,2)
    !      write(*,'(1x,"vvf(",i2,")",2f10.5)') i,ttt(:,i)
    !   end do
    !end if
  ...
  subroutine construct_vion_vh_floc_2( rho, Vion, Vh, force_local, Eh )
    implicit none
    real(8),intent(in)  :: rho(:,:)
    real(8),intent(out) :: Vion(:), Vh(:), force_local(:,:)
    real(8),intent(out) :: Eh
    ...
    !call write_border( 1, " construct_vion_vh_floc_2(start)" )
...
!$OMP end parallel do
    ...
    call rsdft_allreduce_sum( force_local, comm_grid )

    !call watchb( ttmp, ttt(:,8), barrier='on' )

    call rsdft_allreduce_sum( GGVL, comm_fftx )
...
!$OMP end parallel do
  ...
  end subroutine construct_vion_vh_floc_2


  subroutine initial_preparation( ictrl )
    implicit none
