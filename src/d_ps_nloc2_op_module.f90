module d_ps_nloc2_op_module

  implicit none

  private
  public :: d_op_ps_nloc2
  public :: d_op_ps_nloc2_hp
  public :: d_reset_init_flag_ps_nloc2_op
...
contains
  ...
  subroutine d_reset_init_flag_ps_nloc2_op
    implicit none
    init_done = .false.
  end subroutine d_reset_init_flag_ps_nloc2_op


  subroutine d_init_op_ps_nloc2_hp
    !$ use omp_lib
...
!$omp end parallel
  ...
  subroutine d_op_ps_nloc2_hp( tpsi, htpsi, n_in, k_in, s_in )
    use ps_nloc2_variables, only: MJJ, JJP, Mlma, d_uVk, d_uVunk, iuV, &
                                  d_uVunk0, flag_backup_uVunk_ps_nloc2, &
                                  nrlma_xyz, num_2_rank, lma_nsend, nzlma, &
                                  d_sbufnl3, d_rbufnl3, sendmap, recvmap
    ...
    if ( Mlma <= 0 ) return
    if ( .not.init_done ) call d_init_op_ps_nloc2_hp
    ...
    if ( flag_backup_uVunk_ps_nloc2 ) then
      call d_op_ps_nloc2_with_restore_uVunk( htpsi, n, k, s )
      return
    end if
    ...
    do ib = 1, nb
      do lma = ompnzlma1, ompnzlma2
        ...
        do j = 1, MJJ(lma)
          i = JJP(j,lma) - i0 + 1
          tmp_sum = tmp_sum + d_uVk(j,lma,k)*tpsi(i,ib)
        end do
        d_uVunk(lma,ib) = iuV(lma)*dV*tmp_sum
      end do ! lma
    end do ! ib
  ...
  !$omp barrier
        ...
        case(1,3,5)
          j=i+1
          do ib = 1, nb
            d_uVunk0(ompnzlma1:ompnzlma2,ib) = d_uVunk(ompnzlma1:ompnzlma2,ib)
          end do
        case(2,4,6)
          j=i-1
        end select
          ...
          if ( irank >= 0 ) then
            do ib = 1, nb
              ...
              do i1 = omplns1(irank,ompmyrank), omplns2(irank,ompmyrank)
                i2 = i2 + 1
                d_sbufnl3(i2,m,i) = d_uVunk0(sendmap(i1,irank),ib)
              end do
            end do
  !$omp barrier
  !$omp master
            nreq=nreq+1
            call MPI_Isend(d_sbufnl3(1,m,i),lma_nsend(irank)*nb &
  ...
  !$omp master
          if ( jrank >= 0 ) then
            nreq=nreq+1
            call MPI_Irecv(d_rbufnl3(1,m,i),lma_nsend(jrank)*nb &
          ...
          if ( jrank >= 0 ) then
            do ib = 1, nb
              ...
              do i1 = omplns1(jrank,ompmyrank), omplns2(jrank,ompmyrank)
                i2 = i2 + 1
                i3 = recvmap(i1,jrank)
                d_uVunk(i3,ib) = d_uVunk(i3,ib) + d_rbufnl3(i2,m,i)
              end do
            end do
          end if
...
!$omp barrier
      call comm_eqdiv_ps_nloc2_mol( d_uVunk )
    ...
    do ib = 1, nb
      do lma = 1, nzlma
        do j = ompMJJ1(lma,ompmyrank), ompMJJ2(lma,ompmyrank)
          i = JJP(j,lma) - i0 + 1
          htpsi(i,ib) = htpsi(i,ib) + d_uVk(j,lma,k)*d_uVunk(lma,ib)
        end do
      end do
    end do
...
!$omp barrier
  ...
  end subroutine d_op_ps_nloc2_hp


  subroutine d_op_ps_nloc2_with_restore_uVunk( htpsi, n, k, s )
    use rgrid_module, only: Igrid
    use ps_nloc2_variables, only: d_uVk, d_uVunk, JJP, restore_uVunk_ps_nloc2
    implicit none
    real(8),intent(inout) :: htpsi(:,:)
    integer,intent(in) :: n, k, s
    integer :: i,j,ib,nb,lma,i0,nzlma

!    call write_border( 1, " d_op_ps_nloc2_with_restore_uVUnk(start)" )

    nb = size( htpsi, 2 )
    i0 = Igrid(1,0)
    nzlma = size( JJP, 2 )

    call restore_uVunk_ps_nloc2( d_uVunk(:,1:nb), n, k, s )
...
!    call write_border( 1, " d_op_ps_nloc2_with_restore_uVUnk(end)" )

  end subroutine d_op_ps_nloc2_with_restore_uVunk


  subroutine d_init_op_ps_nloc2
    use rgrid_module, only: Igrid
    use ps_nloc2_variables, only: MJJ, JJP
    implicit none
    integer :: ML_0,ML_1,lma,i,j,n,nzlma

    call write_border( 0, " d_init_op_ps_nloc2(start)" )
...
! --- inverse map ( grid label i --> (j,lma) )
    ...
    call write_border( 0, " d_init_op_ps_nloc2(end)" )

  end subroutine d_init_op_ps_nloc2


  subroutine d_op_ps_nloc2( tpsi, htpsi, n, k, s )
    use rgrid_module, only: Igrid, dV
    use rgrid_mol_module, only: iswitch_eqdiv
    use ps_nloc2_variables, only: Mlma, MJJ, JJP, d_uVk, d_uVunk, d_uVunk0, iuV, &
                                  num_2_rank, lma_nsend, d_sbufnl1, d_rbufnl1, &
    ...
    if ( Mlma <= 0 ) return
    if ( .not.init_done ) call d_init_op_ps_nloc2
...
!$omp barrier
    ...
    do ib = nb_0_omp, nb_1_omp
      do lma = nzlma_0_omp, nzlma_1_omp
        d_uVunk(lma,ib)=0.0d0
        do j = 1, MJJ(lma)
          i = JJP(j,lma) - i0 + 1
          d_uVunk(lma,ib) = d_uVunk(lma,ib) + d_uVk(j,lma,k)*tpsi(i,ib)
        end do
        d_uVunk(lma,ib) = iuV(lma)*dV*d_uVunk(lma,ib)
      end do
    end do
...
!$omp barrier
        ...
        case(1,3,5)
          ...
          do ib = nb_0_omp, nb_1_omp
          do lma = nzlma_0_omp, nzlma_1_omp
            d_uVunk0(lma,ib) = d_uVunk(lma,ib)
          end do
          end do
        case(2,4,6)
          j=i-1
...
!$omp barrier
        ...
        do m = 1, nrlma_xyz(i)
          ...
          if ( irank >= 0 ) then
            ...
            do i1 = n_0, n_1
              i2 = i1 + (ib-1)*lmani
              d_sbufnl1(i2) = d_uVunk0(sendmap(i1,irank),ib)
            end do
            end do
!$omp barrier
!$omp master
            nreq=nreq+1
            call MPI_Isend(d_sbufnl1,lmani*nb &
            ,MPI_REAL8,irank,1,comm_grid,ireq(nreq),ierr)
!$omp end master
          end if
!$omp master
          if ( jrank >= 0 ) then
            ...
            nreq=nreq+1
            call MPI_Irecv(d_rbufnl1,lmanj*nb &
            ,MPI_REAL8,jrank,1,comm_grid,ireq(nreq),ierr)
          end if
          call MPI_Waitall(nreq,ireq,istatus,ierr)
!$omp end master
...
!    write(*,*) "time=",et1-et0
  ...
  subroutine comm_eqdiv_ps_nloc2_mol( uVunk )
    use ps_nloc2_variables, only: lma_nsend, d_sbufnl, d_rbufnl, sendmap, recvmap
...
!$omp do
        do i1 = 1, m
          i2 = i1 + (ib-1)*m
          d_sbufnl(i2,irank) = uVunk(sendmap(i1,irank),ib)
        end do
!$omp end do
      end do !ib
!$omp master
      nreq = nreq + 1
      call MPI_Isend(d_sbufnl(1,irank),m*nb,MPI_REAL8,irank,1,comm_grid,ireq(nreq),ierr)
      nreq=nreq+1
      call MPI_Irecv(d_rbufnl(1,irank),m*nb,MPI_REAL8,irank,1,comm_grid,ireq(nreq),ierr)
!$omp end master
    end do !irank
!$omp master
...
!$OMP do
        do i1 = 1, m
          i2 = i1 + (ib-1)*m
          i3 = recvmap(i1,irank)
          uVunk(i3,ib) = uVunk(i3,ib) + d_rbufnl(i2,irank)
        end do
!$OMP end do
      end do !ib
    end do !irank
