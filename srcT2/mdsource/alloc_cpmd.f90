...
subroutine alloc_cpmd
  ...
  use cpmd_variables, only: tau,sig,gam,gamn,scr,wrk,psi_n,psi_v,MBC &
                           ...
                           ,MB_0_SCF,MB_1_SCF
! <MOD 2018/12
  use scalapack_module, only: LLD_R, LLD_C 
! use parallel_module, only: np_band
...
! <MOD 2018/12
     ...
     allocate( tau(MXLLD,MXLLD), sig(MXLLD,MXLLD) ) 
     allocate( gam(MXLLD,MXLLD),gamn(MXLLD,MXLLD) ) 
     allocate( scr(MXLLD,MXLLD), wrk(MXLLD,MXLLD) ) 
  else
...
! <MOD 2018/12
  allocate( psi_v(n1:n2,MB_0_SCF:MB_1_SCF,MBZ_0:MBZ_1,MSP_0:MSP_1) )
  allocate( psi_n(n1:n2,MB_0_SCF:MB_1_SCF,MBZ_0:MBZ_1,MSP_0:MSP_1) )
