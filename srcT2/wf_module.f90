...
!         ,MB_0_WF,MB_1_WF,MK_0_WF,MK_1_WF,MS_0_WF,MS_1_WF,unk,SYStype )
  ...
  SUBROUTINE random_initial_wf
    ...
    real(8) :: u(2)
    logical, parameter :: idebug=.false. ! .true.
        ...
        if (idebug) then
          do n=1,MB_WF
             do i=1,ML_WF
                ...
                if(n>=MB_0_WF.and.n<=MB_1_WF) then
                  if (i>=ML_0_WF.and.i<=ML_1_WF)then
...
!   if (idebug) call check_unk( size(unk,1), size(unk,2), unk(:,:,1,1) )
