...
!   real(8),parameter :: C1=2.14611945579107d0,C2=0.031091d0
    ...
    real(QP) ::         dfz_dz,                 H
    real(QP) ::                                tmp,          Ai
    real(QP) :: dA_dn, dec_dn,           dT_dphi
    real(QP) :: dz_dn(2), cm
!   real(QP) :: dac_drs, deP_drs, deU_drs
    real(QP),allocatable :: rrrr(:,:), rtmp(:)
    real(QP) :: Pi, one, two, fouthr, onethr, zero
    real(QP) :: sevthr, twothr, thrtwo, ThrFouPi
    real(8),allocatable :: f(:),Gf(:)

    real(QP) :: T_zeta(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_fz(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_kf(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_rs(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_rssq(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_ec_U(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_ec_P(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_alpc(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_ec_lda(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dphi_dz(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_phi(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_T(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_A(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_Hs(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_drs_dn(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_deU_dn(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_deP_dn(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dac_dn(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dec_dz(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dH_dA(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dH_dT(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dH_dphi(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_rhoa(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_rhob(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_tho(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_deU_drs(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_deP_drs(rho%g_range%head : rho%g_range%tail)
    real(QP) :: T_dac_drs(rho%g_range%head : rho%g_range%tail)
    integer  :: T_list(1:rho%g_range%tail-rho%g_range%head+1),icount,kkk
#else
    real(QP) :: C1,C2
    real(QP) :: const1, const2, factor
    integer :: i,j,ispin,m,i1,i2,i3,j1,j2,j3,k1,k2,k3,ierr
    ...
    do i=rho%g_range%head,rho%g_range%tail
       ...
       T_rhoa(i) = rho%val(i,1)*factor
       T_rhob(i) = rho%val(i,rho%s_range%size_global)*factor
       T_tho(i) = T_rhoa(i) + T_rhob(i)

       if ( T_tho(i) <= zero_density ) cycle

       T_zeta(i) = ( T_rhoa(i) - T_rhob(i) )/T_tho(i)
       if ( abs(T_zeta(i)) > one ) T_zeta(i)=sign(one,T_zeta(i))
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       T_fz(i) = ( (one+T_zeta(i))**fouthr + (one-T_zeta(i))**fouthr - two)*const1

       T_kf(i) = ( 3.0_QP*Pi*Pi*T_tho(i) )**onethr

       T_rs(i) = ( ThrFouPi/T_tho(i) )**onethr

       T_rssq(i) = sqrt(T_rs(i))

       T_ec_U(i) = -two*A00*( one + alp10*T_rs(i) ) &
            *log( one + one/( two*A00 &
            *(bt10*T_rssq(i) + bt20*T_rs(i) + bt30*T_rs(i)*T_rssq(i) + bt40*T_rs(i)*T_rs(i)) ) )

       T_ec_P(i) = -two*A01*( one + alp11*T_rs(i) ) &
            *log( one + one/( two*A01 &
            *(bt11*T_rssq(i) + bt21*T_rs(i) + bt31*T_rs(i)*T_rssq(i) + bt41*T_rs(i)*T_rs(i)) ) )

       T_alpc(i) = -two*A02*( one + alp12*T_rs(i) ) &
            *log( one + one/( two*A02 &
            *(bt12*T_rssq(i) + bt22*T_rs(i) + bt32*T_rs(i)*T_rssq(i) + bt42*T_rs(i)*T_rs(i)) ) )

       T_ec_lda(i) = T_ec_U(i) - T_alpc(i)*T_fz(i)*const2*(one-T_zeta(i)**4) &
            + ( T_ec_P(i) - T_ec_U(i) )*T_fz(i)*T_zeta(i)**4

       T_phi(i) = 0.5_QP*( (one+T_zeta(i))**twothr + (one-T_zeta(i))**twothr )
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       T_T(i) = Pi*g2/( 16.0_QP*T_phi(i)**2*T_kf(i)*T_tho(i)**2 )
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       T_A(i) = C1/( exp( -T_ec_lda(i)/(C2*T_phi(i)**3) ) - one )
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       T_Hs(i) = C2 * T_phi(i)**3 * log( one + C1*T_T(i)*(one+T_A(i)*T_T(i))/(one+T_A(i)*T_T(i)+(T_A(i)*T_T(i))**2) )

!      Ec = Ec + T_tho(i)*( T_ec_lda(i) + T_Hs(i) )
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       T_drs_dn(i) = -4.0_QP*Pi*T_rs(i)**4/9.0_QP

       tmp = bt10*T_rssq(i) + bt20*T_rs(i) + bt30*T_rs(i)*T_rssq(i) + bt40*T_rs(i)*T_rs(i)
       T_deU_drs(i) = alp10*T_ec_U(i)/( one + alp10*T_rs(i) ) &
            +A00*( one + alp10*T_rs(i) )/T_rssq(i) &
            *( bt10 + two*bt20*T_rssq(i) + 3.0_QP*bt30*T_rs(i) + 4.0_QP*bt40*T_rs(i)*T_rssq(i) ) &
            /( two*A00*tmp*tmp+tmp )

       tmp = bt11*T_rssq(i) + bt21*T_rs(i) + bt31*T_rs(i)*T_rssq(i) + bt41*T_rs(i)*T_rs(i)
       T_deP_drs(i) = alp11*T_ec_P(i)/( one + alp11*T_rs(i) ) &
            +A01*( one + alp11*T_rs(i) )/T_rssq(i) &
            *( bt11 + two*bt21*T_rssq(i) + 3.0_QP*bt31*T_rs(i) + 4.0_QP*bt41*T_rs(i)*T_rssq(i) ) &
            /( two*A01*tmp*tmp+tmp )

       tmp = bt12*T_rssq(i) + bt22*T_rs(i) + bt32*T_rs(i)*T_rssq(i) + bt42*T_rs(i)*T_rs(i)
       T_dac_drs(i) = alp12*T_alpc(i)/( one + alp12*T_rs(i) ) &
            +A02*( one + alp12*T_rs(i) )/T_rssq(i) &
            *( bt12 + two*bt22*T_rssq(i) + 3.0_QP*bt32*T_rs(i) + 4.0_QP*bt42*T_rs(i)*T_rssq(i) ) &
            /( two*A02*tmp*tmp+tmp )

       T_deU_dn(i) = T_deU_drs(i) * T_drs_dn(i)
       T_deP_dn(i) = T_deP_drs(i) * T_drs_dn(i)
       T_dac_dn(i) = T_dac_drs(i) * T_drs_dn(i)
...
!ocl NORECURRENCE
    do kkk=1,icount
       ...
       dfz_dz = fouthr*( (one+T_zeta(i))**onethr - (one-T_zeta(i))**onethr)*const1

       T_dec_dz(i) = -T_alpc(i)*dfz_dz*const2*(one-T_zeta(i)**4) &
               + 4.0_QP*T_alpc(i)*T_fz(i)*const2*T_zeta(i)**3 &
               +(T_ec_P(i)-T_ec_U(i))*( dfz_dz*T_zeta(i)**4 + T_fz(i)*4.0_QP*T_zeta(i)**3 )
!>>
       Ec = Ec + T_tho(i)*( T_ec_lda(i) + T_Hs(i) )
!<<
       if ( abs(T_zeta(i)) == one ) then
          T_dphi_dz(i) = zero
       else
          T_dphi_dz(i) = ( (one+T_zeta(i))**(-onethr)-(one-T_zeta(i))**(-onethr))/3.0_QP
       end if
...
!ocl norecurrence
    do kkk=1,icount
       ...
       tmp = one + T_A(i)*T_T(i) + (T_A(i)*T_T(i))**2

       T_dH_dA(i) = -T_phi(i)**3*C1*C2*T_A(i)*T_T(i)**3*(two+T_A(i)*T_T(i))/(tmp**2+C1*T_T(i)*(one+T_A(i)*T_T(i))*tmp)

       T_dH_dT(i) = T_phi(i)**3*C1*C2*(one+two*T_A(i)*T_T(i))/(tmp**2+C1*T_T(i)*(one+T_A(i)*T_T(i))*tmp)

       T_dH_dphi(i) = 3.0_QP*T_Hs(i)/T_phi(i)
    ...
    do kkk=1,icount
       ...
       dz_dn(1)   = 2.0_QP*T_rhob(i)/T_tho(i)**2
       dz_dn(rho%s_range%size_global) =-2.0_QP*T_rhoa(i)/T_tho(i)**2

       do ispin=rho%s_range%head,rho%s_range%tail

          dec_dn = T_deU_dn(i) - T_dac_dn(i)*T_fz(i)*const2*(one-T_zeta(i)**4) &
               +(T_deP_dn(i)-T_deU_dn(i))*T_fz(i)*T_zeta(i)**4 + T_dec_dz(i)*dz_dn(ispin)

          dA_dn = T_A(i)*(C1+T_A(i))/(C1*C2*T_phi(i)**3) &
               *( dec_dn - 3.0_QP*T_ec_lda(i)/T_phi(i)*T_dphi_dz(i)*dz_dn(ispin)) 
          vc(i,ispin) = vc(i,ispin) + T_ec_lda(i) + T_Hs(i) + T_tho(i)*dec_dn &
               + T_tho(i)*T_dH_dA(i)*dA_dn &
               + T_dH_dT(i)*(-sevthr*T_T(i) - T_tho(i)*two*T_T(i)/T_phi(i)*T_dphi_dz(i)*dz_dn(ispin) ) &
               + T_tho(i)*T_dH_dphi(i)*T_dphi_dz(i)*dz_dn(ispin)

       end do ! ispin

       rtmp(i) = T_dH_dT(i)*Pi/(8.0_QP*T_kf(i)*T_tho(i)*T_phi(i)**2)
    end do ! i
